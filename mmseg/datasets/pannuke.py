...
class PanNukeDataset(CustomDataset):
    ...
    def __init__(
            self,
            # classes=("other", "Inflammatory", "Epithelial", "Spindle-shaped"),    # CoNSeP
            # classes=("epithelial", "lymphocyte", "neutrophil", "macrophage", "ambiguous"),  # monusac
            classes=("Neo", "Inflam", "Connec", "Necros", "Epi"),    # pannuke
            # classes=('Cancer Epithelial', 'Endothelial', 'PVL', 'Epithelial', 'Inflam', 'CAFs'),
            **kwargs,
    ):
        ...
        for img_info in self.img_infos:
            ...
            if seg_map.endswith(".pth"):
                ...
                if self.num_classes == 2:
                    ...
                    type_map = np.where((type_map != 0), 1, type_map)
                    gt["type_map"] = type_map
            elif seg_map.endswith(".npy"):
                inst_map = np.load(seg_map)
                gt = {"inst_map": inst_map}
            elif seg_map.endswith(".mat"):
                results = scipy.io.loadmat(seg_map)
                gt = {"inst_map": results["inst_map"],
                      "type_map": results.get("type_map")}
                # 合并gt中的一些分类
                type_map = gt["type_map"]
    ...
    def evaluate(self, results, metric="mIoU", logger=None, gts=None, **kwargs):
        ...
        ret_metrics_summary = OrderedDict()
        for ret_metric, ret_metric_value in ret_metrics.items():
            ret_metrics_summary[ret_metric] = np.round(ret_metric_value * 100, 3)
        ...
        for key, value in ret_metrics_summary.items():
            eval_results[key] = np.round(np.nanmean(value) / 100.0, 5)
    ...
    def format_results(self, results, **kwargs):
        for file_idx, res in enumerate(results):
            ...
            # 8进程
            # with Pool(8) as pool:
            #     results_single = list(tqdm(pool.starmap(get_inst_info, args),
            #                                total=len(inst_id_list)))
            
            # for inst_id, inst_info in results_single:
