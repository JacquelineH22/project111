...
def get_fast_pq(true, pred, match_iou=0.5):
    ...
    true = np.copy(true).astype(np.int16) 
    pred = np.copy(pred).astype(np.int16)
    true_id_list = list(np.unique(true))
    pred_id_list = list(np.unique(pred))
    ...
    for true_id in true_id_list[1:]:  # 0-th is background
        t_mask = true_masks[true_id]
        pred_true_overlap = pred[t_mask > 0]   # 提取预测结果 pred 中对应于 t_mask 前景区域的值
        pred_true_overlap_id = np.unique(pred_true_overlap)
        pred_true_overlap_id = list(pred_true_overlap_id)
        for pred_id in pred_true_overlap_id:
            if pred_id == 0:  # ignore
...
class CustomDataset(Dataset):
    ...
    def __init__(
        ...
        self.gt_seg_map_loader = (
            ...
            else LoadAnnotationsNpy(**gt_seg_map_loader_cfg)
        )
...
class NucleiDataset(CustomDataset):
    ...
    def __init__(self, **kwargs):
        ...
        if "split" in kwargs:
            kwargs.pop("split")
        super(NucleiDataset, self).__init__(**kwargs)
    ...
    def evaluate(
        self, results, metric="mIoU", logger=None, gt_instance_maps=None, **kwargs
    ):
        ...
        if mmcv.is_list_of(results, np.ndarray) or mmcv.is_list_of(results, str):
            ...
            ret_metrics = self.run_nuclei_inst_stat(results, gt_instance_maps)
    ...
    def run_nuclei_inst_stat(self, results, gt_instance_maps):
        ...
        for res, gt_ins_map in zip(results, gt_instance_maps):
            ...
            gt_ins_map = gt_ins_map.astype(np.int32)
