...
class DefaultFormatBundle(object):
    ...
    def __call__(self, results):
        ...
        if "gt_inst_mask" in results:
            results["gt_inst_mask"] = DC(
                to_tensor(results["gt_inst_mask"]),
                stack=False,
            )
        if "gt_points" in results:
            results["gt_points"] = DC(
                to_tensor(results["gt_points"]),
                stack=False,
            )
        if "center_xy" in results:
            results["center_xy"] = DC(
                to_tensor(results["center_xy"]),
                stack=False,
            )
        if "cell_num" in results:
            results["cell_num"] = DC(
                to_tensor(results["cell_num"]),
                stack=False,
            )
