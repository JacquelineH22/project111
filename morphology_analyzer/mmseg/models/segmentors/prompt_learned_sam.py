...
class PromptLearnedSam(BaseSegmentor):
    def __init__(
        self,
        prompter,
        sam_model,
        ...
        super(PromptLearnedSam, self).__init__(init_cfg)
        ...
        if freeze_sam:
            for m in self.sam.modules():
                m.eval()
                for param in m.parameters():
                    param.requires_grad = False

        # build prompter
        self.prompter = builder.build_backbone(prompter)
    ...
    def forward_train(
        ...
        **kwargs,
        ...
        prompt_pred = out_dict["out"]["prompt_pred"]  # [bs, 1024, 64, 64]

        cell_num = torch.tensor(
            [pp.shape[0] for pp in gt_sam_prompt_points], device=img.device
        )
        gt_sam_prompt_points = torch.cat(gt_sam_prompt_points, dim=0).unsqueeze(
            1
        )  # [num, 1, 1, 2]
        gt_sam_prompt_labels = torch.cat(gt_sam_prompt_labels, dim=0).unsqueeze(
            1
        )  # [num, 1, 1]
    ...
    def encode_decode(self, img, img_metas, **kwargs):
        ...
        if gt_sam_prompt_points is not None:
            ...
            indexes = torch.where(
                ...
                & (gt_sam_prompt_points[..., 0] < x2)
                & (gt_sam_prompt_points[..., 1] > y1)
                & (gt_sam_prompt_points[..., 1] < y2)
            )
            gt_sam_prompt_points = gt_sam_prompt_points[indexes]
            gt_sam_prompt_labels = gt_sam_prompt_labels[indexes]
            gt_sam_prompt_points[:, 0] -= x1
            gt_sam_prompt_points[:, 1] -= y1
        ...
        for batch_idx in range(img.shape[0]):
            ...
            if gt_sam_prompt_points is None:
                if len(pred_points_sam) > 0:
                    pred_points_sam = torch.tensor(pred_points_sam, device=img.device)
                    pred_points_sam = pred_points_sam * 4  # heat_preds为原图的1/4
                    ...
                    sam_output = self.sam(
                        input_img,
                        pred_points_sam.unsqueeze(1).unsqueeze(1),
                        multimask_output=False,
                        cell_num=len(pred_points_sam),
                        prompt_pred=prompt_pred[batch_idx].unsqueeze(0),
                    )
                else:
                    continue
            else:
                if gt_sam_prompt_points.shape[0] > 0:
                    # ----------------------------------#
                    # draw_img = img[0].cpu().numpy()
                    # draw_img = np.ascontiguousarray(np.transpose(draw_img, (1, 2, 0)))
                    # mean = img_metas[0]["img_norm_cfg"]["mean"].reshape(1, 1, 3)
                    # std = img_metas[0]["img_norm_cfg"]["std"].reshape(1, 1, 3)
                    # draw_img = draw_img * std + mean
                    # draw_img = np.clip(draw_img, a_min=0, a_max=255).astype(np.uint8)
                    # for xy in gt_sam_prompt_points:
                    #     xy = (xy.cpu().numpy() / 4.0).astype(np.int32).tolist()
                    #     draw_img = cv2.circle(draw_img, (xy[0], xy[1]), 3, (0, 255, 0), -1)
                    # cv2.imwrite(img_metas[0]["ori_filename"], draw_img)
                    # ----------------------------------#
                    # 使用gt的prompts, 测试上限
                    sam_output = self.sam(
                        img[batch_idx].unsqueeze(0),
                        gt_sam_prompt_points.unsqueeze(1).unsqueeze(1),
                        gt_sam_prompt_labels.unsqueeze(1).unsqueeze(1),
                        multimask_output=False,
                        cell_num=cell_num,
                    )
