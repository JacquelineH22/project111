...
class DAHead(BaseDecodeHead):
    ...
    def losses(self, seg_logit, seg_label, seg_weight=None):
        ...
        loss.update(
            add_prefix(
                super(DAHead, self).losses(pam_cam_seg_logit, seg_label, seg_weight),
                "pam_cam",
            )
        )
        loss.update(
            add_prefix(
                super(DAHead, self).losses(pam_seg_logit, seg_label, seg_weight), "pam"
            )
        )
        loss.update(
            add_prefix(
                super(DAHead, self).losses(cam_seg_logit, seg_label, seg_weight), "cam"
            )
        )
        return loss
